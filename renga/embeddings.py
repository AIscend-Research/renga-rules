...
Kept separate from the Claude calls in llm.py on purpose: rule
...
*labels* still come from Claude (see llm.extract_tags) because free-text
theme extraction is not something a small embedding model can do; but once
we have the labels we embed them locally for lineage clustering (see
provenance.py).
"""
